...
%======================================================================
function [PeopleId, reproductnumtemp]=IntubationProcessWeekend(CommunitiesMat, InfectedId, InfectionPara, MatIdEDirect, awarenessWend, precautionWend,immuneresp)
...
for iDir=1:length(MatIdEDirect)
    ...
    ContactInd(Iac) = [];
    k = length(ContactInd);
%     CommID = 4; % for public area, community id 4
%     ComPara = SimulatedCommPara(CommID);
%     A =
%     exp(-(ComPara.awareness(1)+ComPara.precaution(1)*((s/k)^ComPara.alfa))); % Now we assume awareness and precaution is same for people in the same community.
    A = exp(-(awarenessWend(MatIdEDirect(iDir))+precautionWend(MatIdEDirect(iDir))*((s/k)^1))); % awareness and precaution are dynamically change with days.
